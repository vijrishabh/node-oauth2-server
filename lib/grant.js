...
function extractCredentials (done) {
  ...
  if (this.req.method !== 'POST' ||
      !this.req.is('application/json')) {
    return done(error('invalid_request',
      'Method must be POST with application/x-www-form-urlencoded encoding'));
  }

  // Grant type
  this.grantType = (this.req.body && this.req.body.grant_type) || 'client_credentials';
  ...
  this.client = credsFromHeader(this.req) || credsFromBasic(this.req) || credsFromBody(this.req);
...
function credsFromHeader (req) {
  return new Client(req.headers.client_id, req.headers.client_secret);
}
